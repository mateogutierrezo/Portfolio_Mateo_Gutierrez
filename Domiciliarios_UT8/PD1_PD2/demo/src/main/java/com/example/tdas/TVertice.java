...
public class TVertice<T> implements IVertice {
    ...
    public void bea(Collection<TVertice> visitados) {
        Queue<TVertice> colaVisitados = new LinkedList<>();
        setVisitado(true);
        visitados.add(this);
        colaVisitados.add(this);

        while (!colaVisitados.isEmpty()) {
            TVertice x = colaVisitados.poll();
    ...
    public boolean hayCamino(Comparable etiqueta) {
        setVisitado(true);
        for (TAdyacencia ady : adyacentes) {
            TVertice destino = ady.getDestino();
            if (!destino.getVisitado()) {
                if (destino.getEtiqueta().compareTo(etiqueta) == 0) {
                    return true;
                }
                if (destino.hayCamino(etiqueta)){
                    return true;
                }
            }
        }
